...
def check_file():
    if not os.path.isfile('contas.exe'):
        print "\n>>> Aviso de seguranca:"
        print "O arquivo contas.exe (BIN) nao existe. Vamos criar um agora!"
        file('contas.exe', 'wb').close()


def carregar_contas():
    # Abrir as listas
    arquivo = open('contas.exe', 'rb')
...
print ">>> Bem-vindo ao sistema BancoBIN v2.0 <<<"
